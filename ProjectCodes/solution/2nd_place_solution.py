"""
So here's our second place solution (the kernel we are publishing is almost similar to the winning kernel) The kernel scored 0.38888 on public and 0.38948 on private and run in 2800s

https://www.kaggle.com/mchahhou/second-place-solution

As a preprocessing step, we kept maximum 60 words for all names and descriptions and apply the same cleaning process on all the texts We labelEncoded all brand and categories and concatenated the encoded values with the name feature

We made 4 models : 1) Ridge model : trained on 1-ngrams and custom bigrams. 1-ngrams is using both name and description. Our custom bigrams work as follows : we concatenate name with the first 5 words from description, then apply np.unique() on the list of word to sort them and remove duplicates, then create all 2-way possible combination of words. The ridge model is able to score .418 on public LB
...
def parallelize_dataframe(df, func):
    df_split = np.array_split(df, cores)  # list(), len = indices_or_sections
    pool = Pool(cores)
...
brand_names = df_train.groupby('brand_name').size()  # 获得每个brand对应的个数的pd.Series
