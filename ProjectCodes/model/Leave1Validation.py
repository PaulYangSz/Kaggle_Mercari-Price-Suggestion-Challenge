...
def leave_1_valid_paral(model_class, tuning_params, all_data_df, n_valid, test_ratio, y_col, clf_or_reg):
    """
    Input model and params output the tuning result(best_para_dict, result_df)
    :param model_class: Class of model
    :param tuning_params: {'param1': [value], 'tuning_param2': [value1, ... ,]}
    ...
    for k, v in tuning_params.items():
        if len(v) > 1:
            search_param_list.append(k)
    assert len(search_param_list) > 0
    search_param_list.sort()
    ...
    best_key_score = -99999.0
    best_params_dict = dict()
    # In parallel params every param have same len
    param_v_len = len(tuning_params[search_param_list[0]])
    for param in search_param_list:
        assert param_v_len == len(tuning_params[param])
    tuning_param_values = []
    for i in range(param_v_len):
        current_value = tuple([tuning_params[param][i] for param in search_param_list])
        tuning_param_values.append(current_value)
    start = time.time()
    elapsed = 0
    if n_valid == 1:
        rand_state = [123]
...
def leave_1_valid_random(model_class, tuning_params, n_params, all_data_df, n_valid, test_ratio, y_col, clf_or_reg):
    """
    Input model and params output the tuning result(best_para_dict, result_df)
    :param model_class: Class of model
    :param tuning_params: {'param1': [value], 'tuning_param2': [value1, ... ,]}
