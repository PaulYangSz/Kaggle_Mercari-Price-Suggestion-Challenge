...
# https://www.kaggle.com/isaienkov/rnn-with-keras-ridge-sgdr-0-43553?scriptVersionId=2115363/code
# version: 87
# In[ ]:
import os
import gc
...
tok_raw = Tokenizer()  # �ָ��ı��ɴʣ�Ȼ�󽫴�ת�ɱ���(�ȷִʣ������)
...
MAX_NAME_SEQ = 20 #17 name��תtexts_to_sequences���list����󳤶ȣ�����Ჹ�㣬������ض�
MAX_ITEM_DESC_SEQ = 60 #269, ͬ�ϣ���item_description
MAX_CATEGORY_NAME_SEQ = 20 #8, ͬ�ϣ���category_name�е�
...
MAX_CATEGORY = np.max([train.category.max(), test.category.max()])+1  # LE��������ֵ+1
...
from keras.preprocessing.sequence import pad_sequences  # Ĭ����ǰ�油�㣬����Ĩ��ǰ��
...
X_train = get_keras_data(dtrain)  # name:���ִʱ��pad�б�, item_desc:�����ʱ��pad�б�,
                                  # brand:Ʒ�Ʊ��, category:�����, category_name:���ʱ��pad�б�,
...
def get_model():
    ...
    emb_name = Embedding(input_dim=MAX_TEXT, output_dim=emb_size//3)(name)  # Embedding�������������ֵ�size�ʹ�����len�󣬸���call������indices�����ش�����.
                                                                            # ����TF��embedding_lookup
    ...
    rnn_layer1 = GRU(units=16) (emb_item_desc)  # GRU������һ��cell�����units���Ⱥ󣬸���call���������,������һ��GRU cell�����(��ΪĬ��return_sequences=False)
                                                # rnn_layer1.shape=[None, 16]
    rnn_layer2 = GRU(8) (emb_category_name)
    rnn_layer3 = GRU(8) (emb_name)
    
    #main layer
    main_l = concatenate([  # �����б��е�Tensor������axis���һ�����Tensor
...
submission["price"] = preds[:test_len]*0.86
print('[{}] Finished predicting test set...'.format(time.time() - start_time))
