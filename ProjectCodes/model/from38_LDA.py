...
print('尝试加入LDA特征信息')
TOPIC_N = 20
cvectorizer = CountVectorizer(min_df=100, max_df=0.75, max_features=180000, token_pattern=r"(?u)\S+", ngram_range=(1,2))
desc_cvz = cvectorizer.fit_transform(full_df['item_description'])
print("desc_cvz.shape={}".format(desc_cvz.shape))  # desc_cvz.shape=(2175020, 54747)
elapsed = time_measure("CountVectorizer.fit_transform('desc')", start, elapsed)
lda_model = LatentDirichletAllocation(n_components=TOPIC_N, learning_method='online', max_iter=20, random_state=42)
desc_topics = lda_model.fit_transform(desc_cvz)
for i in range(TOPIC_N):
    full_df['desc_topic{}'.format(i)] = desc_topics[:,i]
elapsed = time_measure("LatentDirichletAllocation.fit_transform('desc_cvz')", start, elapsed)
...
def get_rnn_data(dataset, num_vars_cols):
    X = {
        'name': pad_sequences(dataset.seq_name, maxlen=MAX_NAME_SEQ),
        'item_desc': pad_sequences(dataset.seq_item_description, maxlen=MAX_ITEM_DESC_SEQ),
        'brand_name': np.array(dataset.brand_name),
        'item_condition': np.array(dataset.item_condition_id),
        'num_vars': np.array(dataset[num_vars_cols]),
...
num_vars_cols = ["shipping"]
for i in range(TOPIC_N):
    num_vars_cols.append('desc_topic{}'.format(i))
X_train = get_rnn_data(train, num_vars_cols)
Y_train = train.target.values.reshape(-1, 1)

X_dev = get_rnn_data(dev, num_vars_cols)
Y_dev = dev.target.values.reshape(-1, 1)

X_test = get_rnn_data(test, num_vars_cols)
