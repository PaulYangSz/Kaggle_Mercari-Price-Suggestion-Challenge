...
class SelfLocalRegressor(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, data_reader:DataReader, name_emb_dim=20, item_desc_emb_dim=60, cat_name_emb_dim=20, brand_emb_dim=10,
        ...
        self.emb_GRU_CNN_model = self.get_GRU_CNN_model(data_reader)
    ...
    def get_GRU_CNN_model(self, reader:DataReader):
        # Inputs
    ...
    def fit(self, X, y):
        ...
        K.set_value(self.emb_GRU_CNN_model.optimizer.lr, self.lr_init)
        K.set_value(self.emb_GRU_CNN_model.optimizer.decay, lr_decay)

        # print('~~~~~~~~~~~~In fit() type(X): {}'.format(type(X)))
        keras_X = self.data_reader.get_keras_dict_data(X)
        keras_fit_start = time.time()
        history = self.emb_GRU_CNN_model.fit(keras_X, y, epochs=self.epochs, batch_size=self.batch_size, validation_split=0., # 0.01
    ...
    def predict(self, X):
        ...
        keras_X = self.data_reader.get_keras_dict_data(X)
        gru_y = self.emb_GRU_CNN_model.predict(keras_X, batch_size=70000, verbose=RNN_VERBOSE)
...
if __name__ == "__main__":
    ...
    if LOCAL_FLAG and len(adjust_para_list) > 0:
        ...
        regress_model.emb_GRU_CNN_model.summary(print_fn=RECORD_LOG)
